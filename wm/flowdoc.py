#!/usr/bin/env python3
"""Generates ui/flows.html: data lineage, pseudo-algorithm, execution flow, dependency graph and step tree (Designer-like) of the flow services of a package."""
import html, os, sys, re, glob
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
MODULE = sys.argv[sys.argv.index("--module") + 1] if "--module" in sys.argv else None
if MODULE is None:
    import flows as F      # ETL package (StarSchemaETL) by default; --module <builder> for another package
...
TR = {  # flow comments (flows.py) -> English, for the public page
...
def tr(txt):
    ...
    for k, v in TR.items():  # prefixes (e.g. "un lot par itération, 4 en parallèle ...")
...
m = None
class _M:  # objectToString: actual output name on this IS (see flows.objtostring_output)
    def call(self, *a, **k): return '{"node":{"svc_sig":{"sig_out":{"rec_fields":[{"field_name":"string"}]}}}}'
services = [] if MODULE else ([F.BEGIN_STEP, F.log_step("string"), F.TRUNCATE_STAR,
...
PKG_TITLE, SUFFIX = "StarSchemaETL", ""
if "--module" in sys.argv:   # e.g. --module oef_build: documents the flows of another package (FLOWS list, PKG and UI_DIR constants)
    import importlib
    M = importlib.import_module(sys.argv[sys.argv.index("--module") + 1])
    services, ORDER, PKG_TITLE, SUFFIX = list(M.FLOWS), [x["node_nsName"] for x in M.FLOWS], M.PKG, "-" + getattr(M, "DOC_SUFFIX", M.UI_DIR)
    OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "ui", M.UI_DIR, "flows-en.html" if LANG == "en" else "flows.html")
by = {s["node_nsName"]: s for s in services}
import flowgraph
if "--module" in sys.argv:
    _adp = flowgraph.adapter_infos(M.ADAPTER_FOLDER, getattr(M, "CUSTOM_SQL", ()), getattr(M, "BATCH_INSERT", ()))
    _docs, _trg, _ntf = getattr(M, "DOCTYPES", []), getattr(M, "TRIGGERS", ()), getattr(M, "NOTIFICATIONS", ())
else:
    import adapters as _A
    _adp, _docs, _trg, _ntf = flowgraph.adapter_infos(_A.FOLDER, _A.CUSTOM_SQL, _A.BATCH_INSERT), F.DOCTYPES, (), ()
graph_services, aliases = list(services), {}
if "--module" not in sys.argv:   # the graph shows every deployed service; runPipelineX2 links to the runPipeline tree
    graph_services += [F.API_CHART, F.API_STOP, F.API_RESET, F.API_SOURCE, F.run_pipeline("runPipelineX2", 2)]
    aliases = {"star.etl:runPipelineX2": "star.etl:runPipeline"}
GRAPH_SVG, GRAPH = flowgraph.render(graph_services, adapters=_adp, doctypes=_docs, lang=LANG, triggers=_trg, notifications=_ntf, pkg=PKG_TITLE,
                                    sections=set(by), aliases=aliases)
import flowexec
GRAPH_TITLE = flowgraph.T[LANG]["title"]
GRAPH_HINT = ("Hover a node to highlight its neighbours; click a flow service to open its step tree below. Built from the deployed putNode definitions: "
              f"{len(GRAPH.nodes)} nodes, {len(GRAPH.edges)} links." if LANG == "en" else
              "Survoler un nœud met en évidence ses voisins ; cliquer un flow service ouvre son arbre d'étapes ci-dessous. Généré à partir des définitions "
              f"putNode déployées : {len(GRAPH.nodes)} nœuds, {len(GRAPH.edges)} liens.")
SVC_EN = {
    ...
    "star.api:chart": "Revenue by month (fact_sales ⋈ dim_date) for the UI chart, refreshed less often than the status.",
    "star.api:stop": "Asks the running pipeline to stop (honoured between two batches).",
    "star.api:reset": "Resets the demo: empties the star schema and the log, re-arms the stop flag (dwh.reset_demo()).",
    "star.api:source": "Source table totals (rows, amount, orders, period) for the reconciliation shown by the UI.",
    "star.etl:runPipelineX2": "ETL orchestrator (2 fact batches in parallel): same flow with MAX-THREADS = 2 on the LOOP.",
}
exec_aliases = dict(aliases, **({"star.etl:runPipelineX4": "star.etl:runPipeline"} if "--module" not in sys.argv else {}))   # a single runPipeline line (X2 / X4: same, parallel batches)
EXEC_SVG, EXEC_ROWS = flowexec.render(graph_services, adapters=_adp, doctypes=_docs, lang=LANG, triggers=_trg, notifications=_ntf, sections=set(by), aliases=exec_aliases)
import flowalgo
_desc_en = {s_.get("node_comment", ""): SVC_EN[n] for n, s_ in {x["node_nsName"]: x for x in graph_services}.items() if n in SVC_EN}
def tr_all(txt):   # step comments (TR) and service descriptions (SVC_EN) in English
    return (_desc_en.get(txt) or tr(txt)) if LANG == "en" else txt
ALGO_HTML = flowalgo.render(graph_services, adapters=_adp, doctypes=_docs, lang=LANG, triggers=_trg, notifications=_ntf, sections=set(by), aliases=exec_aliases, tr=tr_all)
ALGO_TITLE, ALGO_HINT = flowalgo.T[LANG]["title"], flowalgo.T[LANG]["hint"]
import flowdata
_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
if "--module" in sys.argv:
    _csql = {f"{M.ADAPTER_FOLDER}:{r[0]}": r[2] for r in getattr(M, "CUSTOM_SQL", ())}
    _bins = {f"{M.ADAPTER_FOLDER}:{r[0]}": (f"{r[2]}.{r[3]}", r[4]) for r in getattr(M, "BATCH_INSERT", ())}
    _schema_files = sorted(f for g in getattr(M, "SCHEMA_GLOBS", []) for f in glob.glob(os.path.join(_root, g)))
    _lg_title = flowdata.T[LANG]["title"]
else:
    _csql = {f"{_A.FOLDER}:{r[0]}": r[2] for r in _A.CUSTOM_SQL}
    _bins = {f"{_A.FOLDER}:{r[0]}": (f"{r[2]}.{r[3]}", r[4]) for r in _A.BATCH_INSERT}
    _schema_files = [os.path.join(_root, "db", "01_schema.sql")]
    _lg_title = flowdata.T[LANG]["title_etl"]
_order = [r["name"] for r in EXEC_ROWS if r.get("name")]
LINEAGE_SVG, LINEAGE_STORY = flowdata.render(graph_services, _csql, _bins, _schema_files, notifications=_ntf, lang=LANG, order=_order, sections=set(by), tr=tr_all)
LINEAGE_HINT = flowdata.T[LANG]["hint"]
EXEC_TITLE = flowexec.T[LANG]["title"]
EXEC_HINT = ("Each line is one service, read left to right in execution order; a package service called by a step is expanded on the line below (once). "
             "Adapter services show their SQL table, ⟨documents⟩ appear where they are produced, dashed boxes are LOOP / BRANCH / CATCH; "
             "utility services (log, timing) are attached to the step they surround. Click a header to open the step tree." if LANG == "en" else
             "Une ligne par service, à lire de gauche à droite dans l'ordre d'exécution ; un service du package appelé par une étape est développé "
             "sur la ligne du dessous (une seule fois). Les services adaptateur indiquent leur table SQL, les ⟨documents⟩ apparaissent là où ils sont "
             "produits, les cadres pointillés sont les LOOP / BRANCH / CATCH ; les services utilitaires (journal, horodatage) sont rattachés à l'étape "
             "qu'ils encadrent. Cliquer un en-tête ouvre l'arbre des étapes.")
parts = []
...
page = f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>{PKG_TITLE} · flow services</title>
<style>
...
main{{padding:16px 24px;display:grid;grid-template-columns:minmax(0,1fr);gap:14px}} .svc{{background:#161c24;border:1px solid #2a3542;border-radius:8px;padding:12px 16px}}
...
.graph .wrap{{overflow:auto;background:#0f1419;border:1px solid #2a3542;border-radius:6px}} .graph svg{{display:block}} .graph svg.fit{{width:100%;height:auto}}
{flowalgo.STYLE}
{flowdata.STYLE}
.graph h2{{display:flex;align-items:center;gap:10px}} details.graph summary{{cursor:pointer;color:#33b1ff;font-size:15px;font-weight:600;margin:0 0 4px}} .graph p.sig{{display:flex;align-items:center;gap:8px;flex-wrap:wrap}} .gtools{{margin-left:auto;display:flex;gap:6px}} .gbtn{{background:#1c2430;color:#8b98a8;border:1px solid #2a3542;border-radius:5px;padding:3px 9px;font:inherit;font-size:11px;cursor:pointer}} .gbtn.on{{color:#33b1ff;border-color:#33b1ff}}
...
</style></head><body><header><h1>{PKG_TITLE + " package · flow services" if LANG == "en" else "Package " + PKG_TITLE + " · flow services"} <small>{"step tree exactly as deployed on the Integration Server (generated by wm/flowdoc.py)" if LANG == "en" else "arbre des étapes tel que déployé sur l'Integration Server (généré par wm/flowdoc.py)"}</small></h1>
<nav>{f'<a href="#data" style="color:#33b1ff">{"data lineage" if LANG == "en" else "cheminement des données"}</a> ' if LINEAGE_SVG else ""}<a href="#algo" style="color:#33b1ff">{"pseudo-algorithm" if LANG == "en" else "pseudo-algorithme"}</a> <a href="#exec" style="color:#33b1ff">{"execution flow" if LANG == "en" else "flux d'exécution"}</a> <a href="#workflow" style="color:#33b1ff">{"dependencies" if LANG == "en" else "dépendances"}</a> {" ".join(f'<a href="#{html.escape(n)}">{html.escape(n.split(":")[1])}</a>' for n in ORDER + [n for n in by if n not in ORDER])}</nav></header>
<main>{f'<section class="svc graph" id="data"><h2>{html.escape(_lg_title)} <span class="gtools"><button class="gbtn" data-fit="1">{"fit to width" if LANG == "en" else "ajuster à la largeur"}</button><button class="gbtn" data-fit="0">{"actual size" if LANG == "en" else "taille réelle"}</button></span></h2><p class="sig">{html.escape(LINEAGE_HINT)}</p><div class="wrap">{LINEAGE_SVG}</div>{LINEAGE_STORY}</section>' if LINEAGE_SVG else ""}
<section class="svc graph" id="algo"><h2>{html.escape(ALGO_TITLE)} <span class="gtools"><button class="gbtn" data-algo="1">{html.escape(flowalgo.T[LANG]["expand"])}</button><button class="gbtn" data-algo="0">{html.escape(flowalgo.T[LANG]["collapse"])}</button></span></h2><p class="sig">{html.escape(ALGO_HINT)}</p>{ALGO_HTML}</section>
<section class="svc graph" id="exec"><h2>{html.escape(EXEC_TITLE)} <span class="gtools"><button class="gbtn" data-fit="1">{"fit to width" if LANG == "en" else "ajuster à la largeur"}</button><button class="gbtn" data-fit="0">{"actual size" if LANG == "en" else "taille réelle"}</button></span></h2><p class="sig">{html.escape(EXEC_HINT)}</p><div class="wrap">{EXEC_SVG}</div></section>
<details class="svc graph" id="workflow"><summary>{html.escape(GRAPH_TITLE)}</summary><p class="sig">{html.escape(GRAPH_HINT)} <span class="gtools"><button class="gbtn" data-fit="1">{"fit to width" if LANG == "en" else "ajuster à la largeur"}</button><button class="gbtn" data-fit="0">{"actual size" if LANG == "en" else "taille réelle"}</button></span></p><div class="wrap">{GRAPH_SVG}</div></details>{"".join(parts)}</main>{flowexec.SCRIPT}{flowalgo.SCRIPT}{flowdata.SCRIPT}</body></html>'''
open(OUT, "w", encoding="utf-8").write(page)
print("écrit", OUT, len(page), "octets,", len(parts), "services")

# ---- Markdown version (docs/flows-trees.md): same trees, as text
import re as _re
def md_node(node, depth=0):
    ...
    return md
MD = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "docs", f"flows-trees{SUFFIX}{'-en' if LANG == 'en' else ''}.md")
lines = ["# Arbres des flow services (générés par wm/flowdoc.py)\n", f"Chaque étape telle que déployée sur l'Integration Server (package {PKG_TITLE}).\n"]
for name in ORDER + [n for n in by if n not in ORDER]:
    s_ = by[name]
