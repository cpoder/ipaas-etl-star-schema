...
- **http://localhost:5555/StarSchemaETL/flows.html**: step tree of every flow service (generated by `wm/flowdoc.py`),
  ...
  Since 2026-09-10 the page opens with four generated views before the step trees, all computed from the deployed definitions:
  **data lineage** (`wm/flowdata.py`: `staging.orders` on the left, one transformation card per target table, dimensions and
  fact table on the right with the rule of every column, e.g. `date_key ← order_date formatted as yyyyMMdd`,
  `customer_key ← key of dim_customer (join customer_code)`, `amount ← quantity × unit_price`; hover a column to light up its
  lineage), a **plain-language pseudo-algorithm** (`wm/flowalgo.py`: trigger by trigger, with the services and documents of
  each step), the **execution flow** (`wm/flowexec.py`: one line per service in execution order) and the **dependency graph**
  (`wm/flowgraph.py`).
